...
const polygonHarness = loadJsonFile.sync(
...
test('Polygon test', t => {
  ...
  if (process.env.REGEN)
    write.sync(path.join(__dirname, 'outputs', 'polygon.json'), output)
  const expected = loadJsonFile.sync(
    path.join(__dirname, 'outputs', 'polygon.json')
  )
  t.deepEqual(output, expected)
})

const polygonCloseInOutHarness = loadJsonFile.sync(
  path.join(__dirname, 'inputs', 'polygonCloseInOut.json')
)
...
test('Polygon test', t => {
  ...
  if (process.env.REGEN)
    write.sync(
      path.join(__dirname, 'outputs', 'polygonCloseInOut.json'),
      output
    )
  const expected = loadJsonFile.sync(
    path.join(__dirname, 'outputs', 'polygonCloseInOut.json')
...
test('Distance parameter works', t => {
  ...
  const dist = distance(
    point(box.geometry.coordinates[0][0]),
    point(
      output.geometry.coordinates[0][output.geometry.coordinates[0].length - 2]
    ),
    {
      units: 'kilometers'
    }
...
test('Distance parameter works with negative value', t => {
  ...
  const output = bufferGeoJSON(box, -2, 'kilometers')
  t.is(
    booleanPointInPolygon(point(output.geometry.coordinates[0][0]), box),
    true
  )
  t.is(output.geometry.coordinates[0].length, 5)

  const dist = distance(
    point(box.geometry.coordinates[0][1]),
    point(output.geometry.coordinates[0][0]),
    {
      units: 'kilometers'
    }
  ...
  t.is(
    dist > expectedDist - 0.00001 && expectedDist < expectedDist + 0.00001,
    true
  )
})
...
test('Polygon with hole negative works', t => {
  const boxWithHole = loadJsonFile.sync(
    path.join(__dirname, 'inputs', 'polygonWithHoleNegative.json')
  )

  const output = bufferGeoJSON(boxWithHole, -2, 'miles', 10)
  if (process.env.REGEN)
    write.sync(
      path.join(__dirname, 'outputs', 'polygonWithHoleNegative.json'),
      output
    )
  const expected = loadJsonFile.sync(
    path.join(__dirname, 'outputs', 'polygonWithHoleNegative.json')
...
test('Polygon with hole negative thats complicated works', t => {
  const boxWithHole = loadJsonFile.sync(
    path.join(__dirname, 'inputs', 'polyWithHoleNegativeTouching.json')
  )

  const output = bufferGeoJSON(boxWithHole, -0.01, 'miles')
  if (process.env.REGEN)
    write.sync(
      path.join(__dirname, 'outputs', 'polyWithHoleNegativeTouching.json'),
      output
    )
  const expected = loadJsonFile.sync(
    path.join(__dirname, 'outputs', 'polyWithHoleNegativeTouching.json')
