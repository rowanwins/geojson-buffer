...
export function bufferLine(geometry, distance, steps, endType, joinType) {
  ...
  return outCoords.length === 1
    ? polygon(outCoords[0])
    : multiPolygon(outCoords)
}
  ...
  coordEach(contour, function(currentCoords, index) {
    ...
    if (index === 0) {
      ...
      if (angleInDegs < 180) {
        ...
        const segmentRev = processSegment(
          currentCoords,
          nextCoords,
          -Math.abs(distance)
        )
        const prevSegmentRev = processSegment(
          prevCoords,
          currentCoords,
          -Math.abs(distance)
        )
